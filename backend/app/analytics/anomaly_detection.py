...
def detect_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["rolling_mean"] = df.groupby("machine_id")["current_power_kw"].transform(
        lambda s: s.rolling(24, min_periods=6).mean()
    )
    df["rolling_std"] = df.groupby("machine_id")["current_power_kw"].transform(
        lambda s: s.rolling(24, min_periods=6).std()
    ).fillna(0)
    df["rolling_mean"] = df["rolling_mean"].fillna(df["current_power_kw"])
